...
Agent/TCP/FullTcp set S_MAX_ [lindex $argv 27]
Agent/TCP/FullTcp/Sack/DTCP set options_ 0
Agent/TCP/FullTcp/Sack/DTCP set flowsize_thre_max_ 501000
Agent/TCP/FullTcp/Sack/DTCP set flowsize_thre_min_ 201000
Agent/TCP/FullTcp/Sack/DTCP set deadline_thre_max_ 0.6
Agent/TCP/FullTcp/Sack/DTCP set deadline_thre_min_ 0.3
Agent/TCP/FullTcp/Sack/DTCP set deadline_wmax_ 0.1
Agent/TCP/FullTcp/Sack/DTCP set deadline_wmin_ 0.05
set myAgent "Agent/TCP/FullTcp/Sack/DTCP";
...
for {set i 0} {$i < $S} {incr i} {
 $ns simplex-link $sender($i) $nqueue 11Gb [expr $host_delay + $mean_link_delay] $switchAlg
 $ns simplex-link $nqueue $sender($i) 11Gb [expr $host_delay + $mean_link_delay] $switchAlg
}
...
set throughputSamplingInterval [expr 0.001]
...
#for {set i 0} {$i < $S} {incr i} {
 ...
 #$ns at 0.0 "$tcps($i) advance-bytes 90000000"
#}

$ns at 0.0 "$tcps(0) advance-bytes 2097152000"
$ns at 0.5 "$tcps(1) advance-bytes 2097152000"
...
set F [lindex $argv 28]
set throughputfile [open $F w]
